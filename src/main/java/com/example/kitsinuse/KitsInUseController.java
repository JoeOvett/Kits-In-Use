...
public class KitsInUseController {
    ...
    public void initialize() {
        ...
        microbiologyButton.getStyleClass().add("LandingButton");
    ...
    private void loadMicrobiologyView(ActionEvent event) {
        try {
            ...
            Scene scene = new Scene(root); // Create Scene directly
            Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
            stage.setScene(scene);
            stage.setWidth(KitsInUse.WIDTH);
            stage.setHeight(KitsInUse.HEIGHT);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
